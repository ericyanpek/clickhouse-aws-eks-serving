...
**要回答的不是"怎么把 ClickHouse 跑上 EKS"，而是：湖仓已是唯一事实来源时，如何用自管 ClickHouse 做一个可重建、低延迟、可运维的 OLAP 加速层。**

**核心命题：先把 ClickHouse 定义成可重建的加速层，再选拓扑和磁盘。** 这个顺序决定后面所有取舍——角色一旦是"可重建"，本地盘丢数据就不等于丢权威事实，EBS 的价值也从"保住数据"变成"少重灌、快换节点"。若 ClickHouse 是唯一数据源，本仓库的恢复模型不适用。

交付一套可评审、可执行的 Terraform + Kubernetes IaC，部署在自有 AWS 账号：
...
适用于数据必须留在自有 VPC、需要控制拓扑与升级节奏、且湖仓已承接唯一数据事实的场景。它**不替代 ClickHouse Cloud**：换来的是控制权，不是更轻的运维。适不适合你取决于六个变量，见 [4.0](#40-先判断这套方案适不适合你)。
...
| 本仓库（EKS + 自管 CHI） | 平台复杂度换可评审 IaC |
| 默认 EBS `1×3` | 约 9% 月成本换重挂卷而非重灌 |
| 本地 NVMe profile | 重灌风险换吞吐余量 |

**存储介质选型是可复现的实测，不是经验判断**——同一 EKS 内并行部署两种介质，产出查询、merge、设备级 I/O、恢复 RTO 和成本五类证据，并标注哪些结论**不**成立。数字见 [4.3](#43-数据卷选型ebs-gp3-与本地-nvme) 与 [11](#11-备份与恢复)。
...
后面几节讨论"选定自管之后怎么选"；更前置的问题是**该不该用**。只有这六个变量会改变结论，团队偏好和机型新旧不会：
...
1. **没有自管硬需求时选 Cloud。** 这套 IaC 换到控制权，不是更轻的运维。
2. **湖仓已是 SoT 时自管加速层成立**——权威 durability 在上游，这边可以用"能重建"换简单。
...
**跑偏信号：** 有人把 ClickHouse 或备份桶称作"数据的家"；或跨 AZ HA 画在图上，实际机型挤在同一个 AZ。
