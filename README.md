...
本项目关注的问题是：当湖仓已经作为唯一事实来源时，如何用自管 ClickHouse 构建可重建、低延迟且具备明确运维路径的 OLAP 加速层。

设计以 ClickHouse 的数据角色为起点，再据此选择拓扑和存储介质。将 ClickHouse 定义为可重建的加速层后，本地盘故障不会造成权威数据丢失；EBS 的主要价值也相应体现为减少数据重载并缩短节点替换时间，而不是保存唯一数据副本。若 ClickHouse 是唯一数据源，本仓库的恢复模型不适用。

仓库提供一套可评审、可执行的 Terraform + Kubernetes IaC，用于在自有 AWS 账号中部署以下组件：
...
集群集合由 Terraform 的 `clickhouse_clusters` map 定义，map key 作为集群标识符。新增集群时增加一个 key 即可。节点组按字符串 key 而非列表序号编址，因此增删集群不会改变其他集群的资源地址。每个集群可独立指定拓扑、存储介质、实例规格和 ClickHouse 版本。

数据卷介质直接影响恢复方式、I/O 上限和成本。基于实测结果，本仓库**默认使用 EBS gp3**：它简化节点故障后的恢复流程，且在 warm 查询场景中与本地 NVMe 性能接近。本地 NVMe 仍作为存储密集型负载的性能 profile 提供。选型依据见 [4.3](#43-数据卷选型ebs-gp3-与本地-nvme)。

本方案适用于数据必须保留在自有 VPC、需要自主控制拓扑与升级节奏，且湖仓已经承接唯一权威数据事实的场景。它**不替代 ClickHouse Cloud**；更高的基础设施控制能力也意味着需要自行承担相应运维工作。适用条件见 [4.0](#40-先判断这套方案适不适合你)。

主要方案的取舍如下：

| 方案 | 主要取舍 |
|---|---|
| ClickHouse Cloud | 基础设施控制能力较少，运维范围较小 |
| EC2 自管 | 避免引入 Kubernetes 控制面 |
| 本仓库（EKS + 自管 CHI） | 增加平台复杂度，以获得可评审的 IaC |
| 默认 EBS `1×3` | 月成本增加约 9%，节点替换时可重挂卷而无需重载数据 |
| 本地 NVMe profile | 提供更高吞吐余量，但节点丢失后需要重载数据 |

存储介质选型基于可复现实验：在同一 EKS 集群内并行部署两种介质，分别记录查询、merge、设备级 I/O、恢复 RTO 和成本，并明确实验结论的适用范围。具体数据见 [4.3](#43-数据卷选型ebs-gp3-与本地-nvme) 与 [11](#11-备份与恢复)。

作为 Cloud 迁 OSS 的 POC，本仓库覆盖 EKS 部署、存储、HA、备份和运维；**不含** Kafka/Flink 双写、历史回填、结果比对和切流，那些属于上游管道。
...
在讨论自管方案内部的选型前，需要先判断自管部署是否适用。以下六个变量会直接影响结论：

| 变量 | 往这边偏时 | 结论 |
|---|---|---|
| **数据角色** | ClickHouse 是唯一数据源，无可重放上游 | **恢复模型不适用**，全部设计前提是可重建 |
| **工作集** | 热数据装得进内存 | 选 EBS gp3，介质差异几乎观察不到 |
| **恢复约束** | 节点替换必须零重建 | EBS + 节点组绑单 AZ 子网，缺一不可 |
| **单查询算力** | 一条查询必须吃多台机器 | 才考虑分片，否则垂直扩容 |
| **组织能力** | 没有平台团队 | Cloud 或 EC2 更合适 |
| **AZ 供给** | 目标机型 3 个 AZ 买不齐 | 跨 AZ HA 只在图纸上，见 [6](#6-前置条件与成本) |

判断规则，按优先级：

1. **没有明确的自管要求时，优先考虑 Cloud。** 本方案提供更高的基础设施控制能力，同时增加运维责任。
2. **湖仓已是 SoT 时，可以将 ClickHouse 作为自管加速层。** 权威数据的持久性由上游保证，ClickHouse 可按可重建组件设计。
3. **单节点还装得下就不要分片。** 加 shard 不搬迁历史数据，re-sharding 远贵于预留容量。
4. **查询以 warm 为主选 EBS gp3。** 恢复从重灌变重挂卷，代价是月成本贵 8.77%。
5. **多集群并存时集群标识必须是 map key**，用列表序号会让增删集群重建其他集群的节点组。

**需要重新评估的情形：** ClickHouse 或备份桶被视为权威数据存储；或者架构声明跨 AZ HA，但目标实例实际集中在同一个 AZ。
...
merge 结果的差异来自持续吞吐，而不是数据量：两种介质**累计搬运的字节数几乎相同**（每节点约 1.0 TiB），持续吞吐分别为 217 MiB/s 和 328 MiB/s。EBS 在 merge 期间设备利用率达到 102%，p95 为 1,130–1,193 MiB/s，接近 1,250 MiB/s 的预置上限；本地 NVMe 的峰值为 2,630–2,815 MiB/s，利用率仍低于 89%。这表明本地 NVMe 在该负载下保留了更大的吞吐余量，而 EBS 的上限受实例通道和预置配置约束。

#### 4.3.2 为什么默认推荐 EBS

在本项目的架构前提下（湖仓是 SoT，ClickHouse 是可重建加速层），EBS 的主要收益体现在运维恢复，而非查询性能：
...
- **warm 查询性能接近。** 实验中两种介质处于同一性能区间；EBS 的差距主要出现在存储密集型场景。

代价是**每月贵 8.77%**，以及**维护窗口需按 1.5 倍规划**（merge 慢 48%）。是否接受这项成本，取决于节点故障后重载约 130 GiB 数据的时间和操作成本是否高于 EBS 的增量费用。

#### 4.3.3 什么时候仍应选本地 NVMe

以下任一条件成立时，应优先评估本地 NVMe：
...
- 需要**持续超过实例 EBS 通道**的吞吐。注意 `r8g.4xlarge` 是突发型规格：1,250 MB/s 每 24 小时只能维持 30 分钟，之后回落到 625 MB/s 基线。若持续负载需要更高吞吐，应改用基线等于最大值的 `r8g.8xlarge`，而不是继续增加卷的预置性能。
...
销毁顺序不是风格问题。**必须先删 CHI，让 EBS CSI 驱动在控制面还活着时回收卷**：数据卷的 `DeleteOnTermination` 是 `false`，控制面一消失 CSI 驱动随之消失，就再没有东西能删卷，卷会永久留在账单上。gp3 类的 `reclaimPolicy` 又是 `Retain`（这正是节点替换能重挂原卷的前提），删 PVC 也不删卷——所以 `teardown.sh` 在删 namespace **之前**读出每个 PV 的 `csi.volumeHandle`，之后逐个删卷并按标签兜底清扫。

teardown 也刻意做成**可重入且不依赖 Kubernetes API**（先把 helm/kubernetes 资源移出 state 再 destroy）。两者都来自实测教训：SSM 隧道中断会让 `helm_release` 卡住数分钟后失败且零资源被销毁，本地 DNS 故障会中断一次删除请求已生效的运行。

### 12.1 停机省钱：先确认存储介质

缩容到 0 对两种 profile 的后果**完全不同**：

| profile | 停节点的后果 |
|---|---|
| `ebs` | 卷独立于实例，节点恢复后重新挂载，**数据保留**；停机期间仍付卷费 |
| `local-nvme` | 数据在 instance store，**实例一停永久消失**，恢复须从健康副本或湖仓重灌 |

所以 `local-nvme` 没有"停机省钱"这个选项——只能销毁并接受重灌，或改用 `ebs`。

#### 12.1.1 缩到零的正确顺序（2026-08-18 实测）

节点组的 `min_size` 是 `1`（Keeper 是 `min=max=1`），而 EKS 拒绝 `desired < min`，**所以要在同一次调用里把 `min` 一起降下来**：
...
**先缩 `system`，再缩数据与 Keeper。** cluster-autoscaler 在 `system` 上；数据节点先缩，它会检测到 Pending 的 ClickHouse Pod 并把节点组扩回来，与缩容对抗。恢复时顺序相反——CoreDNS、operator 和 autoscaler 都在 `system` 上，其余组件在它们就绪前不会收敛。

恢复用 `terraform apply` 最干净，它会把 `min_size` 恢复成声明值。但 ClickHouse 节点组的 `desired_size` 在 `ignore_changes` 之下（运行时交给 autoscaler），apply 后可能仍停在 0，需单独确认。

#### 12.1.2 全量缩零会卡在 PDB，属预期

实测 16 台中有 10 台在 ASG 里停在 `Terminating:Wait` 约 5 分钟。**这不是故障。** 终止生命周期钩子先驱逐 Pod 并遵守 operator 的 `maxUnavailable: 1`；每个池第一个节点排空后，它的 Pod 因所有节点组都在缩零而无处调度、只能 Pending，PDB 再也腾不出配额，剩余驱逐全部阻塞。

钩子 `DefaultResult=CONTINUE`、超时 1800 秒，最终一定自行推进。要立即放行：
...
在"节点本来就要全部消失"的前提下这是安全的：ClickHouse 对进程被杀是崩溃安全的，`ebs` 的数据卷 `Retain` 且独立于实例。但**不要**用在滚动替换场景——那时跳过排空等于放弃 PDB 的保护。
