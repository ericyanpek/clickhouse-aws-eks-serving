...
[中文](./perf-testing-plan.md) · **English**
...
Since access is ClusterIP only, all `clickhouse-benchmark` invocations run as a Kubernetes Job inside the cluster. The service `clickhouse-ch` (ClusterIP) load-balances across all 3 pods.
...
```
Source: [best-practices notes §6](../docs/notes-ck-on-eks-best-practices-2026.en.md)
