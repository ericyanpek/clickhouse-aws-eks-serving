# Multi-Cluster ClickHouse on EKS IaC Implementation Plan

[中文](./2026-08-14-multi-cluster-clickhouse-iac.md) · **English**

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [x]`) syntax for tracking.

**Goal:** Let one EKS bring up one or more mutually independent ClickHouse clusters on demand, each with its own topology, storage, instance size, and version, such that adding or removing any cluster leaves the others untouched.

**Architecture:** Introduce a `clickhouse_clusters` map variable whose key becomes the stable identifier for every derived resource. ClickHouse and Keeper node groups move out of the upstream Altinity module (index-keyed, prone to shifting) into self-managed `aws_eks_node_group` + `for_each` (map-keyed, stable). The upstream module keeps VPC / EKS / addons and the fixed-size shared pools. CHI/CHK change from static YAML into templates rendered by `deploy.sh`.
...
**Design basis:** [`docs/superpowers/specs/2026-08-13-multi-cluster-clickhouse-iac-design.en.md`](../specs/2026-08-13-multi-cluster-clickhouse-iac-design.en.md)

## Preconditions

Verified state before executing this plan:

- AWS is fully cleared: the EKS cluster is deleted, 13 instances terminated, 5 EBS volumes reclaimed, and Terraform state holds 0 resources. A from-scratch apply is possible.
- The S3 backup bucket `clickhouse-eks-ch-backups` is retained and removed from state (0 objects inside).
- Seven files carry **uncommitted changes** (the global `storage_profile` approach): `terraform/{variables,storage,eks,outputs}.tf`, `scripts/{deploy,teardown}.sh`, `manifests/20-clickhouse-chi.yaml`. This plan **does not commit that intermediate state**; Task 1 converts it directly into the map approach.

## Hard Constraints (measured, not negotiable)

1. **The upstream module is index-keyed.** `eks/main.tf:89` keys node groups by list index, so any change in list length or order rebuilds live node groups. ClickHouse/Keeper node groups must therefore move out.
2. **One subnet per AZ.** A gp3 volume is AZ-scoped, so a node group must be pinned to a single subnet for a replacement node to land in the same AZ and reattach the original volume (verified in the P8 drill: 111s reattach, zero rebuild).
3. **Do not hand-write a PodDisruptionBudget.** The operator creates one already (`maxUnavailable: 1`); an overlapping PDB makes pods permanently unevictable.
4. **Teardown must delete the CHI before tearing down the control plane.** Data volumes have `DeleteOnTermination=False`, so if the control plane goes first the EBS CSI driver goes with it and the volumes remain forever with nothing to reclaim them.
5. **Teardown must not depend on the Kubernetes API and must be re-entrant.** Lessons from three real destroys: a dropped tunnel stalls a helm_release for five minutes before failing, and a DNS failure interrupts the run even though its delete requests already took effect.

---

## Task 1: Replace storage_profile with a clickhouse_clusters map
...
- [x] **Step 1: 删除全局 storage_profile 及其衍生变量**
...
- [x] **Step 2: 新增 clickhouse_clusters map 变量**
...
## Task 2: Reduce shared pools to system and bench only
...
## Task 4: Per-cluster StorageClass

**Files:**
- Modify: `terraform/storage.tf`

- [x] **Step 1: 替换为按集群 for_each 的 gp3 class**
...
- [x] **Step 2: 把 local-storage class 与 provisioner 改为条件创建**
...
- [x] **Step 3: 新增 outputs 供 deploy.sh 读取**
...
## Task 5: Template the Keeper manifest
...
## Task 6: Template the ClickHouse CHI manifest
...
- [x] **Step 2: 新增 ZooKeeper path 前缀隔离**
...
## Task 7: Template the backup manifest
...
- [x] **Step 3: 新增 output 供 deploy.sh 读取每集群 role ARN**
...
## Task 8: Loop deploy.sh over clusters
...
## Task 9: Parameterize smoke-test.sh

**Files:**
- Modify: `scripts/smoke-test.sh`

- [x] **Step 1: 接受 namespace 与集群名**
...
- [x] **Step 2: 把硬编码的 namespace 与 cluster 名替换为变量**
...
## Task 10: Per-cluster teardown with no Kubernetes API dependency
...
- [x] **Step 4: 全量路径改为可重入且不依赖 Kubernetes API**
...
## Task 11: Real-deployment verification
...
- [x] **Step 7: 验证 autoscaler 识别与 taint 生效**
