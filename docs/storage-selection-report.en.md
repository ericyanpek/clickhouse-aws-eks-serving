...
**Next step:** reduce gp3 to 20k IOPS while **retaining 1,250 MiB/s** — observed peaks of roughly 15k IOPS plus 120–157 KiB per I/O show the workload is throughput-bound rather than IOPS-bound. That tier costs about $2,180.14/month for two nodes, an 8.77% premium over local NVMe. **Testing 1,000 and 625 MiB/s is no longer recommended:** merge-window device data shows EBS p95 at roughly 1,130–1,193 MiB/s with 102% peak utilization, meaning more than 5% of the merge window runs above 1,100 MiB/s. Dropping to 1,000 MiB/s would clip merge throughput directly and further extend a convergence time that is already 48% slower.

> **Execution status: performed on 2026-08-13.** The basis for this tier is already established by measurement (peak IOPS 14,993 / 9,701, and only 1,415 / 1,428 during the merge window; 120–157 KiB per I/O means saturating 1,250 MiB/s needs only about 8,200 IOPS; the `r8g.4xlarge` EBS sustained baseline is itself 20,000 IOPS, so provisioning beyond it is usable only within the 30-minute-per-24-hour burst allowance). No further measurement was needed to decide. Execution was deliberately deferred until after the HA drill so the merge control baseline stayed intact; the P7/P8 drills completed the same day, so the change went ahead as planned. Both volumes were modified online with `aws ec2 modify-volume`, holding throughput at 1,250 MiB/s. No pod restarted, both volumes stayed `in-use`, both replicas remained byte-identical (999,974,970 rows / 139,911,668,216 bytes), and replication lag stayed at 0. Two-node monthly cost went from $2,380.14 to $2,180.14, reducing the premium over local NVMe from 18.75% to 8.77%, about $2,400 per year. The IaC was updated to match (`terraform/variables.tf` defaults and `terraform.tfvars`), so newly provisioned volumes match the existing ones.
>
> **Effect on later experiments:** every figure in sections 10.2 and 10.4 was collected at 40k IOPS. Because this workload is throughput-bound rather than IOPS-bound (120-157 KiB per I/O; saturating 1,250 MiB/s needs only about 8,200 IOPS), the change is expected not to alter merge or query performance -- but that expectation is **not yet verified by measurement**. Any future merge rerun or symmetric local-NVMe comparison must note the configuration difference rather than subtracting directly from this round's numbers.
>
> Execution notes: the StorageClass parameters (`terraform/storage.tf`) affect newly provisioned volumes only; existing volumes must be changed with `aws ec2 modify-volume`, which is online on Nitro instances and needs no detach or restart, after which Terraform state and the actual configuration diverge until reconciled. Each volume permits at most 4 modifications per rolling 24 hours and the previous one must reach `completed` first, so the change should be made correctly in a single pass. The ratio limits (IOPS ≤ 500 × GiB, throughput ≤ 0.25 MiB/s per provisioned IOPS) are not binding at this tier.

Remaining work: a clean merge rerun with both profiles converging to 1/1, alternating repeat runs, P3b storage-isolated writes, and P7/P8 HA.
