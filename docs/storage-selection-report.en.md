...
> **Execution status: `TODO`, deliberately not performed in this round.** The basis for this tier is already established by measurement (peak IOPS 14,993 / 9,701, and only 1,415 / 1,428 during the merge window; 120–157 KiB per I/O means saturating 1,250 MiB/s needs only about 8,200 IOPS; the `r8g.4xlarge` EBS sustained baseline is itself 20,000 IOPS, so provisioning beyond it is usable only within the 30-minute-per-24-hour burst allowance). No further measurement is needed to decide. Execution is deferred because every measurement in this round was taken on the current 40k IOPS / 1,250 MiB/s configuration, and changing the tier would destroy the control baseline required to reproduce the merge and HA results. Apply it once, after the P7/P8 HA exercises are complete and the cluster no longer needs to stay reproducible.
>
