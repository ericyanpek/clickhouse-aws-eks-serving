...
2. **EBS 已饱和，实例存储仍有余量。** EBS 峰值 utilization 102%、p95 合计约 1,130–1,193 MiB/s，紧贴 1,250 MiB/s 预置上限；实例存储 p95 已达 1,517–1,651、峰值 2,630–2,815 MiB/s，utilization 仍不足 89%。在本轮负载下,实例存储保留了更高的吞吐余量;EBS 上限由实例通道和预置配置共同决定。
...
**适用边界：** warm-cache 与常规并发场景中 EBS 与 NVMe 只有低个位数差异，EBS 更符合固定常驻节点、同 AZ 重挂原卷和降低重灌频率的运维目标。若业务存在频繁绕过 page cache 的大扫描、严格 direct-I/O 尾延迟或持续存储饱和，实测 23%–78% 的差异应纳入存储选型。新增的 merge 证据把这一边界收紧到运维层面：等量工作下 EBS 的 merge 收敛慢 48.0%，因此**维护窗口需按 1.5 倍规划**；若后台 merge 必须在固定夜间窗口内完成，merge 时长可能成为主要约束。
...
> **执行状态：已于 2026-08-13 执行完成。** 该档位的依据已由本轮实测确定（峰值 IOPS 14,993 / 9,701，merge 窗口仅 1,415 / 1,428；每次 I/O 120–157 KiB，打满 1,250 MiB/s 只需约 8,200 IOPS；`r8g.4xlarge` 的 EBS 持续 baseline 本身就是 20,000 IOPS，预置超出部分只在 30 分钟/24 小时突发额度内可用），无需再补测即可决策。原计划推迟到 HA 演练之后执行，以保住 merge 复现所需的对照基线；P7/P8 演练已于同日完成，因此按计划执行。两卷均由 `aws ec2 modify-volume` 在线调整，吞吐保持 1,250 MiB/s 不变，全程 Pod 未重启、卷保持 `in-use`、两副本数据逐字节一致（999,974,970 行 / 139,911,668,216 字节）、复制延迟为 0。两节点月成本 $2,380.14 → $2,180.14，相对本地盘溢价 18.75% → 8.77%，年省约 $2,400。IaC 已同步（`terraform/variables.tf` 默认值与 `terraform.tfvars`），因此新建卷与现有卷配置一致。
>
> **对后续实验的影响：** 第 10.2 与 10.4 节的全部数据采集于 40k IOPS 配置。由于该负载受吞吐而非 IOPS 限制（每次 I/O 120–157 KiB，打满 1,250 MiB/s 仅需约 8,200 IOPS），预期本次调整不改变 merge 或查询性能；但这一预期**尚未实测验证**。若未来重跑 merge 或做本地 NVMe 对称对比，须注明配置差异，不得与本轮数字直接相减。
>
